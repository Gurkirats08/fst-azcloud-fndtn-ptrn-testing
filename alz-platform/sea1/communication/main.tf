...
module "communication_user_assigned_identity" {
  source              = "Azure/user-assigned-identity/azurerm"
  name                = var.communicationuan
...
module "storage_account" {
  ...
  user_assigned_identity_id       = module.communication_user_assigned_identity.id
  identity_type                   = "UserAssigned"
  identity_ids                    = [module.communication_user_assigned_identity.id]
  public_network_access_enabled   = false
  allow_nested_items_to_be_public = false
  shared_access_key_enabled       = each.value.shared_access_key_enabled
  queue_encryption_key_type       = "Account"
  table_encryption_key_type       = "Account"
  depends_on                      = [module.resource_group, module.communication_user_assigned_identity]
}

# virtual networks
module "communication_vnet_module" {
  for_each                     = var.communicationVirtualNetworks
...
#needed
# subnets
module "communication_subnet_module" {
  for_each                  = var.communicationSubnets
  ...
  depends_on                = [module.resource_group, module.communication_vnet_module]
  service_endpoints         = ["Microsoft.KeyVault"]
}


# Virtual Hub
module "communication_vhub_module" {
  for_each                   = var.communicationVHub
  source                     = "Azure/virtual-hub/azurerm"
  name                       = each.value.name
  resource_group_name        = var.resourceGroups["netRG"].name
  location                   = each.value.location
  vwan_id                    = module.communication_vwan_module[each.value.vwan_key].virtual_wan_id
  ...
  vnet_connections           = {for conn_key, conn in try(each.value.vnet_connections, {}) : conn_key => {
    vnet_id                  = module.communication_vnet_module[conn.vnet_key].id
  } }
  depends_on                 = [module.communication_vwan_module, module.resource_group]
}
