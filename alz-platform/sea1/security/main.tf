...
module "security_user_assigned_identity" {
...
module "storage_account" {
  ...
  user_assigned_identity_id       = module.security_user_assigned_identity.id
  identity_type                   = "UserAssigned"
  identity_ids                    = [module.security_user_assigned_identity.id]
  public_network_access_enabled   = false
  allow_nested_items_to_be_public = false
  shared_access_key_enabled       = each.value.shared_access_key_enabled
  queue_encryption_key_type       = "Account"
  table_encryption_key_type       = "Account"
  depends_on                      = [module.resource_group, module.security_user_assigned_identity]
}

# virtual networks
module "security_vnet_module" {
  for_each                     = var.securityVirtualNetworks
...
#needed
# subnets
module "security_subnet_module" {
  for_each                  = var.securitySubnets
  ...
  depends_on                = [module.resource_group, module.security_vnet_module]
  service_endpoints         = ["Microsoft.KeyVault"]
}


# Virtual Hub
module "security_vhub_module" {
  for_each                   = var.securityVHub
  source                     = "Azure/virtual-hub/azurerm"
  name                       = each.value.name
  resource_group_name        = var.resourceGroups["netRG"].name
  location                   = each.value.location
  vwan_id                    = module.security_vwan_module[each.value.vwan_key].virtual_wan_id
  ...
  vnet_connections           = {for conn_key, conn in try(each.value.vnet_connections, {}) : conn_key => {
    vnet_id                  = module.security_vnet_module[conn.vnet_key].id
  } }
  depends_on                 = [module.security_vwan_module, module.resource_group]
}
